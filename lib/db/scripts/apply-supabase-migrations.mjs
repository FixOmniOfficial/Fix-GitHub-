...
async function establishBaseSchema() {
  ...
  if (problems.length) {
    console.log(`Base schema incomplete (${problems.length} problem(s)); applying repair migration…`);
    const repairSql = await readFile(path.join(migrationsDirectory, "0004_base_repair.sql"), "utf8");
    await client.query("BEGIN");
    try {
      await client.query(repairSql);
      await client.query("COMMIT");
      console.log("Base schema repair applied successfully");
    } catch (error) {
      await client.query("ROLLBACK");
      throw new Error(`Base schema repair failed: ${error.message}`);
    }
    const remainingProblems = await baseSchemaProblems();
    if (remainingProblems.length) {
      throw new Error(`Base schema repair incomplete: ${remainingProblems.join("; ")}`);
    }
  }
